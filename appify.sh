...
Options:
  ...
  -a, --app     Path of the app to 'appify' (required)
...
Copyright:
  ...
  Modified by Jerry Lin <https://github.com/rainchei>
EOF
  exit 1
}
...
cp -v "$APP" "$APPDIR/MacOS/$APPNAME"
# Delete the shortest match of string in $var from the end: ${var%string}
rsync -av "${APP%/*}/" "$APPDIR/MacOS" --exclude $APP
